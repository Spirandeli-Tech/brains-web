...
import addressPrClient, { computeStats } from "@/lib/clients/address-pr";
import type { FixRun, RunStatus } from "@/lib/clients/address-pr";
...
export function AddressPrPage() {
  ...
  const fetchRuns = useCallback(async (silent = false) => {
    ...
    try {
      const data = await addressPrClient.listRuns();
      setRuns(data);
    } catch (err) {
      if (!silent) {
        message.error(err instanceof Error ? err.message : "Failed to load runs");
      }
    } finally {
  ...
  const handleApprove = async (
    ...
    try {
      await addressPrClient.approveStep(runId, stepId, fixPlan);
  ...
  const handleIterate = async (runId: string, stepId: string, notes: string) => {
    ...
    try {
      await addressPrClient.iterateStep(runId, stepId, notes);
      await fetchRuns(true);
    } catch (err) {
  ...
  const handleCancel = async (runId: string) => {
    ...
    try {
      await addressPrClient.cancelRun(runId);
      message.success("Run cancelado");
      await fetchRuns(true);
    } catch (err) {
      message.error(err instanceof Error ? err.message : "Failed to cancel run");
    } finally {
  ...
  const handleRestart = async (runId: string) => {
    ...
    try {
      await addressPrClient.restartRun(runId);
      message.success("Run reiniciado");
      await fetchRuns(true);
    } catch (err) {
      message.error(err instanceof Error ? err.message : "Failed to restart run");
    } finally {
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader
        ...
        subtitle="Cole a URL de um PR — o runner lê os comentários, propõe as correções e consulta você antes de aplicar e responder"
        actions={
      ...
      <FixStats stats={stats} activeFilter={filter} onFilter={handleStatFilter} />
          ...
          <Empty
            ...
            description={
              filter === "all"
                ? "Nenhum run ainda."
                : filter === "active"
                ? "Nenhum run ativo. Cole uma URL de PR e clique em Endereçar."
                : `Nenhum run ${filter.replace("_", " ")}.`
            }
          >
